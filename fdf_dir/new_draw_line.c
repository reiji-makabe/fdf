...
static void	draw_loop_pos(t_data *img, t_draw *draw);
static void	draw_loop_neg(t_data *img, t_draw *draw);
...
void	draw_line(t_map *p_0, t_map *p_1, t_data *img)
{
	...
	put_value_t_color(&draw, draw.color);
	if (draw.x_and_y_is_pos)
		draw_loop_pos(img, &draw);
	else
		draw_loop_neg(img, &draw);
}
