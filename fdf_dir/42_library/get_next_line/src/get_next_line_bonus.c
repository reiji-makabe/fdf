...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/03 18:11:22 by rmakabe           #+#    #+#             */
/*   Updated: 2022/11/16 04:27:38 by rmakabe          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || read(fd, 0, 0) < 0)
		return (NULL);
	buf[fd] = read_until_getnewline(fd, buf[fd]);
	if (!buf[fd])
		return (NULL);
	line = return_one_line(buf[fd]);
	buf[fd] = restart_buf(buf[fd]);
	return (line);
}
